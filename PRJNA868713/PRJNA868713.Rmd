...
# select runs from PRJNA868713
cts <- cts %>% filter(Run %in% SRRs$Run) 
# Il y a plusieurs formats de GeneID
# Certains commencent par NP ou YP ou sont de la forme rna-b4413 ou gnl|b0240|CDS251 ou b4583	
...
cts_per_run <- cts_per_run %>%
  group_by(time) %>%
  summarise(counts = sum(counts), .groups = "drop") %>%
  as.data.frame()
colnames(cts_per_run)[colnames(cts_per_run) == "Library.Name"] <- "Run"
```

```{r}
# Bar plot number of gene counts per run
par(mgp = c(3, 0.2, 0))
barplot(cts_per_run$counts, main = "Number of reads per run", names.arg = cts_per_run$time, ylab = "# reads", col = "darkblue", border = NA, las = 2, cex.names = 0.7, cex.axis = 0.8)
...
plot_CLR <- function(protein, ds_type) {
  ...
  protein_clr <- protein_clr %>%
    ...
    mutate(time_rep = str_remove_all(time_rep, " min")) %>%
...
deseq_volcano_c_i <- function(t) {
  ...
  cols <- c("0 min_1", "0 min_2", "0 min_3",
    paste0(t, " min_1"),
    paste0(t, " min_2"),
    paste0(t, " min_3")
  )
  ...
  plot <- ggplot(res, aes(x = log2FoldChange, y = -log10(padj), color = ds)) +
    ...
    xlim(-15, 15) +
    ylim(0, 20) +
    annotate("text", x=-10, y=15, label = paste0(t, " min")) +
    theme_bw()
...
plot4 <- deseq_volcano_c_i(4) 
plot7 <- deseq_volcano_c_i(7) 
plot20 <- deseq_volcano_c_i(20) 

plot1 + plot4 + plot7 + plot20 + plot_layout(nrow = 2) 
```