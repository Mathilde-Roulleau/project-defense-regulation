...
SRRs <- SRRs %>%
  mutate(State = ifelse(str_detect(strain, "P3"), "infected", "control"),
...
SRRs <- SRRs %>%
  ...
  ungroup() %>%
  mutate(time_rep = paste0(State, "_", time, "_", rep))
...
# select runs from PRJNA307603
cts <- cts %>% filter(Run %in% SRRs$Run) %>%
  mutate(Gene_origin = ifelse(grepl("^CP", Chr), "Bacteria", "Phage"))
cts$Geneid <- sub("^cds-", "", cts$Geneid)
cts$Geneid <- sub("^rna-", "", cts$Geneid)

# they switched Bacteria and Phage genes in SRR3084371
#cts[(cts$Run == "SRR3084371" & cts$Gene_origin == "Phage"),]$Gene_origin <- "Bacteria"
#cts[(cts$Run == "SRR3084371" & cts$Gene_origin == "Bacteria"),]$Gene_origin <- "Phage"
